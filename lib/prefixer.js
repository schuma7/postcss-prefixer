...
const prefixer = (options) => {
  ...
  return {
    ...
    Once(css) {
      ...
      css.walkRules((rule) => {
        /* The Tokenizer mishandles escaped commas (\2c) in complex CSS rules. Replacing them with placeholders before processing and restoring after seems to resolve the issue */
        const parsed = Tokenizer.parse(rule.selector.replace(/\\2c/g, ","));
        const selector = interateSelectorNodes(parsed, { prefix, ignore });

        /* eslint no-param-reassign: "off" */
        rule.selector = Tokenizer.stringify(selector).replace(/,/g, "\\2c");
      });
    },
  };
};
